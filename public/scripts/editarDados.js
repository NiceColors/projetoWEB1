const habilitaEdicao = (element) => {
  let inputElement = element.parentNode.childNodes[1].childNodes[3]
...
submit.addEventListener('click', (e) => {
  const userId = document.getElementById('userId').innerHTML.slice(4)
  const form = document.getElementsByTagName('form')
  const fields = ["nome", "email", "nascimento", "cpf", "tel", "senha"];
  let inputs = {}
  ...
  putAjax.onload = () => {
    if (putAjax.status == 200) {
      let response = JSON.parse(putAjax.responseText);
      // alert(response.message);
      location.href = response.redirect;
    }
  }

  putAjax.open('PUT', `/atualizarDados/${userId}/`)

  putAjax.setRequestHeader('Content-Type', 'application/json')

  putAjax.send(JSON.stringify(inputs));
